/* Target support for C++ classes on OS/2 - based on winnt-cxx.
...
bool
i386_emx_type_dllimport_p (tree decl)
{
...
bool
i386_emx_type_dllexport_p (tree decl)
{
  gcc_assert (TREE_CODE (decl) == VAR_DECL 
              || TREE_CODE (decl) == FUNCTION_DECL);
...
{
  if (i386_emx_type_dllimport_p (decl))
    DECL_DLLIMPORT_P (decl) = 1;
}

static inline void maybe_add_dllexport (tree decl) 
{
  if (i386_emx_type_dllexport_p (decl))
    {   
...
void
i386_emx_adjust_class_at_definition (tree t)
{
  tree member;
